...
from views.tournament_views import show_current_round_info, get_round_results, display_leaderboard
from controllers.player_controllers.player_controller import sort_players_alphabetically
from views.player_menu_views import display_list_of_players
from views.round_views import display_round_history
from controllers.tournament_controllers.tournament_management_controller import recreate_tournament
...
def show_players_alphabetic(tournament):
    found_data = find_tournament(tournament[0]["name"], tournament[0]["dates"])
...
def show_leaderboard(tournament):
    ...
    players_and_points_sorted = sorted(players_and_points, key=lambda player: player[1], reverse = True)
